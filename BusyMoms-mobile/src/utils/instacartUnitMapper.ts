...
export class InstacartUnitMapper {
  ...
  static getPreferredUnitForCategory(category: string, measurementType: MeasurementType): string {
    const categoryPrefs =
      this.INSTACART_PREFERRED_UNITS[category] || this.INSTACART_PREFERRED_UNITS.other;
  ...
  static validateInstacartMeasurement(
    quantity: number,
    unit: string
  ): {
    isValid: boolean;
  ...
  static getCategoryPreferredUnits(category: string): {
    volume: string;
    weight: string;
    count: string;
  } {
    const prefs = this.INSTACART_PREFERRED_UNITS[category];
    if (prefs) {
      return prefs as { volume: string; weight: string; count: string };
    }
    return this.INSTACART_PREFERRED_UNITS.other as { volume: string; weight: string; count: string };
  }
}
