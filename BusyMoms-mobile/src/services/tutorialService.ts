...
export type TutorialName = 'dashboard' | 'calendar' | 'family_hub' | 'shopping' | 'tasks';
...
export async function getTutorialProgress(tutorialName: TutorialName): Promise<boolean> {
  ...
  if (!user) {
    return true; // Skip tutorial if not logged in
  }
...
export async function resetAllTutorials(): Promise<void> {
  ...
  const tutorials: TutorialName[] = ['dashboard', 'calendar', 'family_hub', 'shopping', 'tasks'];
