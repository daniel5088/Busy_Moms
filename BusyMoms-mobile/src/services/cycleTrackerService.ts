...
class CycleTrackerService {
  ...
  async callEdgeFunction(action: string, cycleData: CycleInsightsRequest): Promise<any> {
    ...
    const EDGE_FUNCTION_URL = `${config.supabaseUrl}/functions/v1/cycle-tracker`;
    ...
    const response = await fetch(EDGE_FUNCTION_URL, {
      ...
      headers: {
        ...
        'x-correlation-id': `${Date.now()}-${Math.random()}`,
      },
      body: JSON.stringify({
        action,
        user_id: user.id,
...
export const cycleTrackerService = new CycleTrackerService();
